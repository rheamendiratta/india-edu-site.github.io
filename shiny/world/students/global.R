...
# ── Plot metadata ──────────────────────────────────────────────────────────────
PLOT_META <- list(
  GER        = list(title="Gross Enrolment Ratio (GER)",            y_label="GER (%)", has_levels=TRUE,  wb_levels=c("Pre-primary","Primary","Secondary","Tertiary"), udise_id="4010", reverse_scale=FALSE, description="Total enrolment as % of official school-age population. May exceed 100% due to over/under-age pupils."),
  NER        = list(title="Net Enrolment Rate (NER)",               y_label="NER (%)", has_levels=TRUE,  wb_levels=c("Primary","Secondary"), udise_id="4011", reverse_scale=FALSE, description="Enrolment of official school-age pupils only, as % of that population."),
  ...
  NIR        = list(title="Net Intake Rate, Primary",               y_label="NIR (%)", has_levels=FALSE, wb_levels="Primary", udise_id=NULL, reverse_scale=FALSE, description="New Grade 1 entrants of official school-entry age as % of that age group."),
  GIR        = list(title="Gross Intake Ratio, Primary",            y_label="GIR (%)", has_levels=FALSE, wb_levels="Primary", udise_id=NULL, reverse_scale=FALSE, description="All new Grade 1 entrants regardless of age, as % of official school-entry age group."),
  GPI        = list(title="Gender Parity Index (GPI of GER)",       y_label="GPI (F/M)", has_levels=TRUE, wb_levels=c("Primary","Secondary","Tertiary"), udise_id="4032", reverse_scale=FALSE, description="Female-to-male enrolment ratio. Below 1 = male advantage; above 1 = female advantage."),
  ...
  LAYS       = list(title="Learning-Adjusted Years of Schooling",   y_label="LAYS (years)", has_levels=FALSE, wb_levels="Primary", udise_id=NULL, reverse_scale=FALSE, description="Expected years of school discounted by learning quality."),
  ...
  LIT_YTH_GPI= list(title="Youth Literacy Gender Parity Index",     y_label="GPI (F/M)", has_levels=FALSE, wb_levels="Primary", udise_id=NULL, reverse_scale=FALSE, description="Female-to-male ratio of youth literacy rates."),
